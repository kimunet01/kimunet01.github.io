---
title: "#1 침대 상호작용 수정"
date: 2026-02-22 04:30:00 +0900
categories: [DevLog, The-Bear] # [대분류, 소분류] 순서입니다.
tags: [Unity]
---

`❗ 저번 주 회의 결과를 바탕으로 침대 상호작용 액션을 바꾸게 되었음.  `
<br>

**🔴 기존 방법**
<div style="background: #fff; border: 1px solid #ddd; padding: 10px; display: flex; gap: 10px;">
  <div style="flex: 1; text-align: center;">
    <img src="/assets/img/posts/2026-02-22-the-bear/Standing_point_before.PNG" style="width: 100%; height: 200px; object-fit: cover;">
    <small>Standing Point</small>
  </div>
  <div style="flex: 1; text-align: center;">
    <img src="/assets/img/posts/2026-02-22-the-bear/Bed_LookATPosition.PNG" style="width: 100%; height: 200px; object-fit: cover;">
    <small>Look At Point</small>
  </div>
</div>

> 상호작용시에 StandingPoint 로 이동하여 LookAtPoint를 보며 암전이 되고 다시 밝아지는 형식

![기존 작동](/assets/img/posts/2026-02-22-the-bear/Bed_interaction_0.gif)
<br>

 **🟡 개선 목표**

 ![지향점](/assets\img\posts\2026-02-22-the-bear\Bed_interaction_wanted.PNG)
 `이런식으로 캐릭터가 누운상태로 암전이 진행되고, 다시 밝아질때 원상태로 돌아와야함.`

<br>

 **🔎 구현 전략 수립**

 `기존코드`
```csharp
IEnumerator SleepRoutineCoroutine(BedInteractable bed)
{
    isHandlingRoutine = true;
    moveVelocity = Vector3.zero;

    // 1. 침대 위치로 이동 및 시선 고정
    float elapsed = 0;
    float moveDuration = 1.0f;
    Vector3 startPos = transform.position;
    Quaternion startRot = transform.rotation;

    while (elapsed < moveDuration)
    {
        elapsed += Time.deltaTime * 2.0f;
        float t = elapsed / moveDuration;
        transform.position = Vector3.Lerp(startPos, bed.sleepPoint.position, t);
        
        // 시선 처리 (누운 자세 연출)
        Vector3 targetDir = (bed.lookAtPoint.position - playerCamera.transform.position).normalized;
        transform.rotation = Quaternion.Slerp(startRot, Quaternion.LookRotation(new Vector3(targetDir.x, 0, targetDir.z)), t);
        yield return null;
    }

    // 2. 암전
    if (GameManager.Instance != null) dayTextUI.text = GameManager.Instance.GetNextDayText(); 

    elapsed = 0;
    while (elapsed < bed.fadeDuration)
    {
        elapsed += Time.deltaTime;
        sleepCanvasGroup.alpha = Mathf.Clamp01(elapsed / bed.fadeDuration);
        yield return null;
    }

    // 3. 대기
    yield return new WaitForSeconds(bed.blackScreenHoldTime);

    // 4. 화면 밝아지기
    elapsed = 0;
    while (elapsed < bed.fadeDuration)
    {
        elapsed += Time.deltaTime;
        sleepCanvasGroup.alpha = Mathf.Clamp01(1 - (elapsed / bed.fadeDuration));
        yield return null;
    }

    isHandlingRoutine = false;
}
```

```
해당 방식에서
1️⃣ StandPoint를 SleepPoint로 바꿔 침대의 내부공간으로 설정
2️⃣ 스크립트에서 캐릭터를 움직이고 각도를 회전
3️⃣ WakeUpPoint를 새로 만들어서 일어날 위치 설정
```
<br>

`💭 이 방식에서 일어 날 수 있는 오류`
```
캐릭터를 회전시킬때 Rigid와 함께 회전이 되는것이 아님.
따라서 캐릭터가 충돌때문에 이상하게 움직일수도 있겠다는 생각이 들었음.
```

`💡 해결 방안`
```
두가지 방법이 떠올랐음.
1️⃣ 침대에 상호작용 할 때 적용되는 물리 엔진을 꺼버리고 끝날때 다시 복구
2️⃣ 캐릭터를 회전시킬 때 Rigid도 같이 회전
```

`✅ 선택 방법`
```
Rigid랑 같이 회전하게 되면 암전이 끝나고 화면이 밝아질 때 다시 Rigid를 원상 복구하는 코드가 필요하고,
위치의 세밀한 조정이 필요할 수 있을것 같음.
➡ 스위치를 끄고 켜듯이 물리 엔진을 끄고 켜는 방법 선택.
```
<br>

`🟠 수정 코드`
```csharp
IEnumerator SleepRoutineCoroutine(BedInteractable bed)
{
    isHandlingRoutine = true;
    moveVelocity = Vector3.zero;

    // 물리 엔진 끄기
    if (controller != null) controller.enabled = false;

    // 1. 침대 눕기
    float elapsed = 0;
    float moveDuration = 1.0f;
    Vector3 startPos = transform.position;
    Quaternion startRot = transform.rotation;

    Quaternion targetRot = Quaternion.Euler(bed.sleepRotation);

    while (elapsed < moveDuration)
    {
        elapsed += Time.deltaTime;
        float t = elapsed / moveDuration;
        transform.position = Vector3.Lerp(startPos, bed.sleepPoint.position, t);
        transform.rotation = Quaternion.Slerp(startRot, targetRot, t);
        yield return null;
    }

    // 2. 암전
    if (GameManager.Instance != null) dayTextUI.text = GameManager.Instance.GetNextDayText(); 
    
    elapsed = 0;
    while (elapsed < bed.fadeDuration)
    {
        elapsed += Time.deltaTime;
        sleepCanvasGroup.alpha = Mathf.Clamp01(elapsed / bed.fadeDuration);
        yield return null;
    }

    // 3. 대기
    yield return new WaitForSeconds(bed.blackScreenHoldTime);

    // 4. 캐릭터 복귀
    transform.rotation = Quaternion.Euler(bed.wakeUpRotation); 
    transform.position = bed.wakeUpPoint.position; 

    // 5. 화면 밝아지기
    elapsed = 0;
    while (elapsed < bed.fadeDuration)
    {
        elapsed += Time.deltaTime;
        sleepCanvasGroup.alpha = Mathf.Clamp01(1 - (elapsed / bed.fadeDuration));
        yield return null;
    }

    // 물리 엔진 켜기
    if (controller != null) controller.enabled = true;
    isHandlingRoutine = false;
}
```
![수정1 작동화면](/assets/img/posts/2026-02-22-the-bear/Bed_interaction_1.gif)
`뭔가 딱딱하고 부자연스러운 것 같음을 느낌`

`💡 해결 방안`

```
Lerp는 선형보간이기 때문에 SmoothStep을 사용해 가속이랑 감속을 주면 될 것 같음.
일어났을때의 시야도 살짝 보정해 주면 좋을 것 같음.
```

`🟢 최종 코드`
```csharp
IEnumerator SleepRoutineCoroutine(BedInteractable bed)
{
	isHandlingRoutine = true;
	moveVelocity = Vector3.zero;

    // 물리 엔진 끄기
	if (controller != null) controller.enabled = false;

	// 1. 침대 눕기
	float elapsed = 0;
	float moveDuration = 1.0f;
	Vector3 startPos = transform.position;
	Quaternion startRot = transform.rotation;

	Quaternion targetRot = Quaternion.Euler(bed.sleepRotation);

	while (elapsed < moveDuration)
	{
		elapsed += Time.deltaTime;
		float t = elapsed / moveDuration;
		float smoothT = Mathf.SmoothStep(0f, 1f, t); 

		transform.position = Vector3.Lerp(startPos, bed.sleepPoint.position, smoothT);
		transform.rotation = Quaternion.Slerp(startRot, targetRot, smoothT);
		yield return null;
	}

	// 2. 암전
	if (GameManager.Instance != null) dayTextUI.text = GameManager.Instance.GetNextDayText(); 
	
	elapsed = 0;
	while (elapsed < bed.fadeDuration)
	{
		elapsed += Time.deltaTime;
		sleepCanvasGroup.alpha = Mathf.Clamp01(elapsed / bed.fadeDuration);
		yield return null;
	}

	// 3. 대기
	yield return new WaitForSeconds(bed.blackScreenHoldTime);

	// 4. 캐릭터 복귀
	transform.rotation = Quaternion.Euler(bed.wakeUpRotation); 
	transform.position = bed.wakeUpPoint.position; 
	currentCameraRotationX = 10.8f;
	playerCamera.transform.localEulerAngles = new Vector3(currentCameraRotationX, 0, 0);

	// 5. 화면 밝아지기
	elapsed = 0;
	while (elapsed < bed.fadeDuration)
	{
		elapsed += Time.deltaTime;
		sleepCanvasGroup.alpha = Mathf.Clamp01(1 - (elapsed / bed.fadeDuration));
		yield return null;
	}

	// 물리 엔진 켜기
	if (controller != null) controller.enabled = true;
	isHandlingRoutine = false;
}
```

`⭕ 최종 실행 결과`
![최종 실행 결과](/assets/img/posts/2026-02-22-the-bear/Bed_interaction_final.gif)

<br>

`📝 느낀점`
```
코드가 너무 지저분하게 휘갈겨져있어서 수정하기 너무 힘들었음;
근데 결과물은 일단 만족만족 대만족
```